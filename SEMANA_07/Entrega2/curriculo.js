...
// Função que faz uma requisição para a API local de habilidades e monta uma lista no currículo com o nome delas.
function carregarHabilidades() {
    const xhttp = new XMLHttpRequest();

    xhttp.onload = function () {
        const arrayHabilidades = JSON.parse(this.responseText);
        document.getElementById("listaHabilidades").innerHTML = arrayHabilidades.map(habilidade => `<li>${habilidade.nome}</li>`).join(""); 
    }

    xhttp.open("GET", "http://localhost:3000/habilidades", true);
    xhttp.send();
}
